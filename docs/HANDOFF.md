...
## Статус
**ПАУЗА — ожидание железа (уточнено 2026-08-27).**

- 0.96" OLED пришёл — слишком мелкий для корпуса указателя (как часы).
- Перезаказ: **OLED/дисплей ~2.8"**.
- Рассматривается временно **ILI9342** (TFT), но на Nano полный кадр убьёт RAM — только draw без framebuffer.
- Прошивка UART/JSN и UI-логика ёмкости готовы; под новый дисплей — смена драйвера/геометрии.

Когда будет 2.8" (или решим пилотить ILI9342) — продолжаем отладку.
