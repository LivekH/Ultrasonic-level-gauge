...
### Если `no.rx` / прочерки
1. **AUTO** шлёт кадр **только когда меняешь setpoint стрелками ВО ВРЕМЯ симуляции** (не до старта).
2. Или поставь **MANUAL** — скетч шлёт `0x55` каждые 200 мс.
3. На экране диагностика: `no.rx` = байт не доходит; `bd.fr` = байты есть, кадр битый; справа `b.. f..`.
4. Проверь, что в Arduino залит свежий hex со `SENSOR_UART_HW 1`.
