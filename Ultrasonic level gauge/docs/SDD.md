...
## UI (OLED)
- Ёмкость без верхней крышки, отступы 4 px.
- Не заливка, а **ватерлиния** (горизонталь зеркала воды).
- По центру ёмкости объём **`XX.YY`** м³ (например `8.56` = 8 м³ + 560 л).
- Справа шкала: **0, 4, 8, 12** м³.
- Без анимации (память Nano).
